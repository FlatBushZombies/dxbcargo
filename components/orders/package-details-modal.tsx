...
export function PackageDetailsModal({ pkg, onClose, onEdit }: PackageDetailsModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm animate-in fade-in duration-200">
      <div className="bg-white rounded-3xl shadow-2xl w-full max-w-2xl animate-in fade-in zoom-in-95 duration-200 max-h-[90vh] overflow-hidden flex flex-col">
        {/* Header */}
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-6">
          ...
          <div>
            ...
            <div className="bg-slate-50 rounded-2xl p-4 space-y-3">
              <div className="flex items-center gap-3">
                <div className="w-8 h-8 rounded-lg bg-[#1B0A2E]/5 flex items-center justify-center">
                  <User className="w-4 h-4 text-[#1B0A2E]" />
                </div>
                <div>
                  <p className="text-xs text-slate-400">Name</p>
                  <p className="text-sm font-semibold text-[#1B0A2E]">{pkg.customer_name}</p>
                </div>
              </div>
              <div className="flex items-center gap-3">
                <div className="w-8 h-8 rounded-lg bg-[#1B0A2E]/5 flex items-center justify-center">
                  <Phone className="w-4 h-4 text-[#1B0A2E]" />
                </div>
                <div>
                  <p className="text-xs text-slate-400">Phone</p>
                  <p className="text-sm font-semibold text-[#1B0A2E]">{pkg.customer_phone}</p>
                </div>
              </div>
              {pkg.customer_email && (
                <div className="flex items-center gap-3">
                  <div className="w-8 h-8 rounded-lg bg-[#1B0A2E]/5 flex items-center justify-center">
                    <Mail className="w-4 h-4 text-[#1B0A2E]" />
                  </div>
                  <div>
                    <p className="text-xs text-slate-400">Email</p>
                    <p className="text-sm font-semibold text-[#1B0A2E]">{pkg.customer_email}</p>
                  </div>
                </div>
              )}
            </div>
          ...
          <div>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="bg-slate-50 rounded-xl p-3">
                ...
                <p className="text-sm font-semibold text-[#1B0A2E]">{pkg.origin} → {pkg.destination}</p>
              </div>
              <div className="bg-slate-50 rounded-xl p-3">
                <div className="flex items-center gap-2 mb-1">
                ...
                <p className="text-sm font-semibold text-[#1B0A2E]">{pkg.transport_type}</p>
              </div>
              {pkg.batch_number && (
                <div className="bg-slate-50 rounded-xl p-3">
                  <div className="flex items-center gap-2 mb-1">
                  ...
                  <p className="text-sm font-semibold text-[#1B0A2E]">{pkg.batch_number}</p>
                </div>
              )}
              <div className="bg-slate-50 rounded-xl p-3">
                <div className="flex items-center gap-2 mb-1">
                ...
                <p className="text-sm font-semibold text-[#1B0A2E]">{pkg.ctn_quantity}</p>
              </div>
              {pkg.weight && (
                <div className="bg-slate-50 rounded-xl p-3">
                  <div className="flex items-center gap-2 mb-1">
                  ...
                  <p className="text-sm font-semibold text-[#1B0A2E]">{pkg.weight} kg</p>
                </div>
              )}
              {pkg.price && (
                <div className="bg-slate-50 rounded-xl p-3">
                  ...
                  <p className="text-sm font-semibold text-[#1B0A2E]">{pkg.currency} {pkg.price}</p>
                </div>
              )}
            </div>
            {pkg.description && (
              <div className="bg-slate-50 rounded-xl p-3 mt-3">
                ...
                <p className="text-sm text-[#1B0A2E]">{pkg.description}</p>
              </div>
            )}
          </div>
                  ...
                  return (
                    <div key={event.id} className="flex gap-3">
                      ...
                      <div className={`flex-1 pb-4 ${index === 0 ? '' : 'opacity-70'}`}>
                        <div className="bg-slate-50 rounded-xl p-3">
                          <p className="text-sm font-semibold text-[#1B0A2E]">{statusInfo?.label || event.status}</p>
                          {event.description && (
        ...
        <div className="p-6 border-t border-slate-100 flex gap-3">
          ...
          {onEdit && (
            <Button onClick={onEdit} className="flex-1 h-12 rounded-xl bg-[#1B0A2E] hover:bg-[#3B1A5C]">
              Edit Package
            </Button>
          )}
        </div>
